...
Trying to get the OpenSCAD CLI to make a bunch of versions of this with different parameters; this command is working:

/Applications/OpenSCAD.app/Contents/MacOS/OpenSCAD -D KnobHeight=20 -D TaperPercentage=30 -D CapType=2 -o knob.stl CustomizableKnob.scad

The -D flags are used to change the values of variables in the design.




I used ChatGPT to generate the script to use for loops to run through a list of possible values. My prompt to ChatGPT:

"I have a terminal command that works, "/Applications/OpenSCAD.app/Contents/MacOS/OpenSCAD -D KnobHeight=20 -D TaperPercentage=30 -D CapType=2 -o knob.stl CustomizableKnob.scad". I now want to write a loop that will change some of the variables I'm feeding that function so that I, for instance, get knobs with heights of 16, 18, 20, 22, 24, and 26, and taper percentages of 0, 5, 10, 15, 20, 25, 30, 35, 40, and 50. The files that are created should have sequential values, e.g. knob1.stl, knob2.stl, etc."


I had a few back-and-forths to fiddle with it but here is the final script I am running, called generateKnobs.sh:

//script start

    #!/bin/bash
    
    # Set path to OpenSCAD binary and source file
    OPENSCAD="/Applications/OpenSCAD.app/Contents/MacOS/OpenSCAD"
    SCADFILE="CustomizableKnob.scad"
    
    # Loop over KnobHeight values
    for height in 15 20 25 30; do
      # Loop over TaperPercentage values
      for taper in 0 10 20 30 40 50; do
        for captype in 1 2; do
          
          # Construct output filename with parameter values
          outfile="knob_h${height}_t${taper}_c${captype}.stl"
    
          # Run OpenSCAD command
          "$OPENSCAD" -D KnobHeight=$height -D TaperPercentage=$taper -D CapType=$captype -o "$outfile" "$SCADFILE"
...
- Robert Zacharias, rzachari@andrew.cmu.edu 8/1/25
released under a CC-BY license, following original author's move (see below)


The actual source for the knob generator below is "Customizable Knob!" from charliearmorycom, dated Feb. 25, 2013, available as thing #54024 at Thingiverse: <https://www.thingiverse.com/thing:54024>. The original author released it under a CC-BY license.
